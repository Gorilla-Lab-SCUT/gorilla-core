...
def _test_handler(file_format, test_obj, str_checker, mode='r+'):
    # dump to a string
    dump_str = gorilla.dump(test_obj, file_format=file_format)
    str_checker(dump_str)

    # load/dump with filenames
    tmp_filename = osp.join(tempfile.gettempdir(), 'gorilla_test_dump')
    gorilla.dump(test_obj, tmp_filename, file_format=file_format)
    assert osp.isfile(tmp_filename)
    load_obj = gorilla.load(tmp_filename, file_format=file_format)
    ...
    with tempfile.NamedTemporaryFile(mode, delete=False) as f:
        tmp_filename = f.name
        gorilla.dump(test_obj, f, file_format=file_format)
    assert osp.isfile(tmp_filename)
    with open(tmp_filename, mode) as f:
        load_obj = gorilla.load(f, file_format=file_format)
    assert load_obj == test_obj
    os.remove(tmp_filename)

    # automatically inference the file format from the given filename
    tmp_filename = osp.join(tempfile.gettempdir(),
                            'gorilla_test_dump.' + file_format)
    gorilla.dump(test_obj, tmp_filename)
    assert osp.isfile(tmp_filename)
    load_obj = gorilla.load(tmp_filename)
    assert load_obj == test_obj
    os.remove(tmp_filename)
...
def test_exception():
    ...
    with pytest.raises(ValueError):
        gorilla.dump(test_obj)

    with pytest.raises(TypeError):
        gorilla.dump(test_obj, 'tmp.txt')
...
def test_list_from_file():
    ...
    filelist = gorilla.list_from_file(filename)
    assert filelist == ['1.jpg', '2.jpg', '3.jpg', '4.jpg', '5.jpg']
    filelist = gorilla.list_from_file(filename, prefix='a/')
    assert filelist == ['a/1.jpg', 'a/2.jpg', 'a/3.jpg', 'a/4.jpg', 'a/5.jpg']
    filelist = gorilla.list_from_file(filename, offset=2)
    assert filelist == ['3.jpg', '4.jpg', '5.jpg']
    filelist = gorilla.list_from_file(filename, max_num=2)
    assert filelist == ['1.jpg', '2.jpg']
    filelist = gorilla.list_from_file(filename, offset=3, max_num=3)
    assert filelist == ['4.jpg', '5.jpg']


def test_dict_from_file():
    filename = osp.join(osp.dirname(__file__), 'data/mapping.txt')
    mapping = gorilla.dict_from_file(filename)
    assert mapping == {'1': 'cat', '2': ['dog', 'cow'], '3': 'panda'}
    mapping = gorilla.dict_from_file(filename, key_type=int)
    assert mapping == {1: 'cat', 2: ['dog', 'cow'], 3: 'panda'}
