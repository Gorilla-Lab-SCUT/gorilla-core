...
def test_assert_dict_contains_subset():
    ...
    assert gorilla.assert_dict_contains_subset(dict_obj, expected_subset)
...
def test_assert_attrs_equal():
    ...
    # case 1
    assert gorilla.assert_attrs_equal(TestExample, {
        'a': 1,
        'b': ('wvi', 3),
        'c': [4.5, 3.14]
    })
    ...
    # case 2
    assert not gorilla.assert_attrs_equal(TestExample, {
        'a': 1,
        'b': ('wvi', 3),
        'c': [4.5, 3.14, 2]
    })
...
def test_assert_dict_has_keys(obj, expected_keys, ret_value):
    assert gorilla.assert_dict_has_keys(obj, expected_keys) == ret_value
...
def test_assert_keys_equal(result_keys, target_keys, ret_value):
    assert gorilla.assert_keys_equal(result_keys, target_keys) == ret_value


@pytest.mark.skipif(torch is None, reason='requires torch library')
def test_assert_is_norm_layer():
    # case 1
    assert not gorilla.assert_is_norm_layer(nn.Conv3d(3, 64, 3))

    # case 2
    assert gorilla.assert_is_norm_layer(nn.BatchNorm3d(128))

    # case 3
    assert gorilla.assert_is_norm_layer(nn.GroupNorm(8, 64))

    # case 4
    assert not gorilla.assert_is_norm_layer(nn.Sigmoid())
...
def test_assert_params_all_zeros():
    ...
    assert gorilla.assert_params_all_zeros(demo_module)

    nn.init.xavier_normal_(demo_module.weight)
    nn.init.constant_(demo_module.bias, 0)
    assert not gorilla.assert_params_all_zeros(demo_module)
...
def test_check_python_script(capsys):
    gorilla.check_python_script('./tests/data/scripts/hello.py zz')
    captured = capsys.readouterr().out
    assert captured == 'hello zz!\n'
    gorilla.check_python_script('./tests/data/scripts/hello.py agent')
    captured = capsys.readouterr().out
    assert captured == 'hello agent!\n'
    # Make sure that wrong cmd raises an error
    with pytest.raises(SystemExit):
        gorilla.check_python_script('./tests/data/scripts/hello.py li zz')
