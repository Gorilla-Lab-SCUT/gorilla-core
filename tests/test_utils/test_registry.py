...
def test_multi_scope_registry():
    ...
    HOUNDS = gorilla.Registry('dogs', parent=DOGS, scope='hound')
    ...
    LITTLE_HOUNDS = gorilla.Registry('dogs', parent=HOUNDS, scope='little_hound')
    ...
    MID_HOUNDS = gorilla.Registry('dogs', parent=HOUNDS, scope='mid_hound')
...
def test_build_from_cfg():
    BACKBONES = gorilla.Registry('backbone')
    ...
    model = gorilla.build_from_cfg(cfg, BACKBONES)
    assert isinstance(model, ResNet)
    assert model.depth == 50 and model.stages == 4

    cfg = dict(type='ResNet', depth=50)
    model = gorilla.build_from_cfg(cfg, BACKBONES, default_args={'stages': 3})
    assert isinstance(model, ResNet)
    ...
    cfg = dict(depth=50)
    model = gorilla.build_from_cfg(
        cfg, BACKBONES, default_args=dict(type='ResNet'))
    assert isinstance(model, ResNet)
    assert model.depth == 50 and model.stages == 4

    cfg = dict(depth=50)
    model = gorilla.build_from_cfg(cfg, BACKBONES, default_args=dict(type=ResNet))
    assert isinstance(model, ResNet)
    assert model.depth == 50 and model.stages == 4
    ...
    with pytest.raises(TypeError):
        cfg = dict(type='ResNet', depth=50)
        model = gorilla.build_from_cfg(cfg, BACKBONES, default_args=1)
    ...
    with pytest.raises(TypeError):
        cfg = dict(type='ResNet', depth=50)
        model = gorilla.build_from_cfg(cfg, BACKBONES, default_args=0)
