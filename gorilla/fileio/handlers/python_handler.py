...
class PythonHandler(BaseFileHandler):
    def load_from_fileobj(self, file):
        raise NotImplementedError(
            f"can not load python from TextIOWrapper, "
            f"please load python file by its path")
    ...
    def dump_to_fileobj(self, obj, file, **kwargs):
        raise NotImplementedError(
            f"can not dump the obj as python file")

    def dump_to_str(self, obj, **kwargs):
        raise NotImplementedError(
            f"can not dump the obj as string in python file")
    ...
    # if file_format not in file_handlers and file_format != "py":
    ...
    #         temp_config_name = os.path.basename(temp_config_file.name)
    #         # Substitute predefined variables
    #         shutil.copyfile(file, temp_config_file.name)

    #         temp_module_name = os.path.splitext(temp_config_name)[0]
    #         sys.path.insert(0, temp_config_dir)
    #         validate_py_syntax(file)
    #         mod = import_module(temp_module_name)
    #         sys.path.pop(0)
    #         obj = {
    #             name: value
